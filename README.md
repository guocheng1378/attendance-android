...
## 工资口径
```
日薪       = (月薪 + 奖金) ÷ 当月天数   // 含奖金，按自然月天数摊；奖金不再单独另加
应出勤天数 = 当月总天数 − 全员没来天数 − 2   （最少 1 天）
                                    // 只用于扣减阈值判定，不参与日薪
出勤折算   = 全天×1 + 半天×0.5
扣减天数   = 出勤 < 应出勤÷2 → 2 天；出勤 < 应出勤 → 1 天；否则 0   // 先判一半，顺序不能反
gross      = round千( min(出勤折算, 当月天数) × 日薪 )   // min 是脏数据护栏，同时保证 gross ≤ 月薪+奖金
扣减       = round千( 日薪 × 扣减天数 )                 // 与日薪同口径，同样含奖金
实发 net   = max(gross − 扣减, 0)                       // 当月总收款
应发 payable = net − round千(预支)                       // 可为负 = 欠款
round千(v) = 四舍五入到 1000 的整数倍                     // (v / 1000.0).roundToLong() * 1000.0
```
例：月薪 4,000,000 ₭、2026-01（31 天）、无停工 → 日薪 4,000,000 ÷ 31 = 129,032 ₭、应出勤 29 天。
满勤（29 个全天）= 29 × 日薪 = 3,742,000 ₭，**满勤不等于满薪**：拿满 4,000,000 ₭ 需要出勤满 31 天。
加 200,000 ₭ 奖金后日薪摊成 135,484 ₭，满勤 3,929,000 ₭；缺 1 天（28 个全天）扣 1 天 135,000 ₭ → 实发 3,659,000 ₭。
...
日历与闰年、非法年月抛异常、应出勤兜底（只影响扣减阈值，不参与日薪）、日薪含奖金按自然月天数摊、
满勤≠满薪（29/31 天 → 3,742,000 ₭）、超勤钳到当月天数、半天折算、扣减阈值边界（14/15/28/29 天）、
gross/扣减/预支各自千位取整一次、预支与欠款、零月薪不炸。断言与上节公式逐条对齐，改口径时必须同步改这里。
